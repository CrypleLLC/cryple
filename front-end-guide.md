...
The API surface is split into five domains: `auth`, `users`, `secrets` (the legacy-item store), `recovery` (guardians, seed recovery, PIN reset) and `succession` (beneficiaries, inheritance shares, release votes).

**Every endpoint answers for the caller as an _owner_ or as a _guardian_. "Heir" is not a role this API serves yet.** An owner manages their own vault, heirs and guardians; a guardian accepts invitations, submits recovery shares, votes on a PIN reset and votes on a release. There is no call by which a person who has been _named_ an heir can discover that they are one, see what is wrapped for them, or — after a release — fetch the wrapped keys and claim it. The first half of that is deliberate and permanent (see [§12](./front-end-endpoints.md#12-succession-endpoints)); the second half is real but unbuilt, because it depends on on-chain release state that nothing mirrors into the database yet. **Do not plan an heir-facing screen against this API.** Everything else in the seven-step product lifecycle is buildable today.
...
23. **Two enum values are schema-only, and two more are on-chain-only.** `completed` on recovery sessions is written by nothing. `released` and `cancelled` on the release trigger are on-chain transitions that only the (not-yet-built) indexer will produce, which is also why `released_at` never appears. Do not build UI that waits for a state this API cannot reach ([§13](./front-end-endpoints.md#13-enumerations)).
