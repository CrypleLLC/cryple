...
Cryple is a **digital inheritance protocol**. Users store client-side-encrypted "legacy items" (seed phrases, credentials, letters) that are released to heirs after prolonged inactivity, via guardians and an on-chain dead man's switch.
...
The API surface is split into seven domains: `auth`, `users`, `secrets` (the legacy-item store), `notes`, `documents` (long-form writing as a sealed snapshot plus an append-only delta log), `recovery` (guardians, seed recovery, PIN reset) and `succession` (beneficiaries, inheritance shares, anchored leaf sets, release votes, and the heir's read path).

**A caller answers as an _owner_, a _guardian_, or — since 2026-08-20 — an _heir_.** An owner manages their own vault, heirs and guardians; a guardian accepts invitations, submits recovery shares, votes on a PIN reset and votes on a release; an heir reads what was left to them, and only after the owner's dead man's switch has released on-chain.

**The heir role is half deliberate silence and half real endpoints, and the split matters.**

- **Before release, nothing.** There is no call by which a person who has been _named_ an heir can discover that they are one. That is permanent by design, not a gap: `GET /succession/inheritances` omits an unreleased inheritance **entirely** rather than reporting it as pending, because an heir who knows they are named can correlate the owner's public on-chain check-in cadence. An empty listing is the normal answer and carries no information. **Never build a screen that hints an inheritance might exist.**
- **After release, six routes** under `/succession/inheritances/…` ([§12](./front-end-endpoints.md#12-succession-endpoints)): the listing, the items left to the caller with their wrapped keys, each item's ciphertext, a document's deltas, and the Merkle leaf sets needed to prove any of it. Verification runs in **your** client against the chain — the API returns data, never a verdict.

Everything else in the seven-step product lifecycle is buildable today.
...
| -------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | Creates something that did not exist, or casts a vote/submission into a running process.                              | `/sign-up`, `/sign-in`, `/auth/verify`, `/secrets`, `/recovery/guardians/invite`, `/recovery/request`, `/recovery/submit`, `/auth/pin-reset/request`, `/auth/pin-reset/vote`, `/users/second-factor`, `/succession/beneficiaries`, `/succession/shares`, `/succession/votes` |
...
| `DELETE` | Removes.                                                                                                              | `/secrets/{id}`, `/users`, `/recovery/guardians/{id}`, `/succession/beneficiaries/{id}`, `/succession/shares/{id}`                                                                                                                                                           |
...
| Code  | Means                                                                                                                       | Where                                                                                                                                                                                                                                                                                                             |
| ----- | --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `201` | **Something was created by this call.**                                                                                     | `POST /sign-up` (new account), `POST /secrets`, `POST /recovery/guardians/invite`, `POST /recovery/request`, `POST /auth/pin-reset/request` (new request), `POST /succession/beneficiaries`, `POST /succession/shares`                                                                                            |
| `200` | A read, a state transition that reports its consequences, or a create-or-return call that **returned** rather than created. | every `GET`, `PUT /recovery/setup`, `POST /succession/votes`, `POST /auth/pin-reset/vote`, `DELETE /recovery/guardians/{id}`, `DELETE /secrets` (batch), `POST /sign-up` on an existing account, `POST /auth/pin-reset/request` on an already-open request, `POST /secrets` replaying an `id` that already exists |
| `204` | Succeeded, and there is **nothing to tell you**. No body at all.                                                            | `DELETE /secrets/{id}`, `DELETE /users`, `DELETE /succession/beneficiaries/{id}`, `DELETE /succession/shares/{id}`, `PATCH /recovery/guardians/{id}/accept`, `POST /recovery/submit`, `POST /users/second-factor`, `PUT /users/password`, `PATCH /auth/pin-reset/revoke`, `PATCH /auth/pin-reset/confirm`         |
...
| `DELETE /secrets/{id}` · `DELETE /succession/beneficiaries/{id}` · `DELETE /succession/shares/{id}` | ⚠️ `404 NOT_FOUND`              | The first call succeeded. Treat `404` on a retry as **success**, not as a missing resource.                                                                                                                                                                       |
...
| `POST /succession/beneficiaries` · `POST /succession/shares` · `POST /succession/votes`             | ✅ safe                         | The same row (`ON CONFLICT DO UPDATE`). `/shares` re-wraps the DEK you sent.                                                                                                                                                                                      |
...
>
> One argument list also changed the same day: `succession-release-vote` now signs the **`release_cycle`** after the owner's `user_address`. A vote signed the old way is refused. See [`POST /succession/votes`](./front-end-endpoints.md#post-successionvotes).

**The rule the API follows**, so you can predict any endpoint without looking it up:

> The JWT authorizes **reads and additions**. Anything that **destroys or replaces** existing data, and anything touching the **guardian / inheritance / sharing graph**, needs an action signature — plus `password` when the signing account is in Paranoid Mode.

So: `GET` anything and `POST /secrets` need only the token. Every `DELETE`, `PUT /recovery/setup`, `PUT /users/password`, and every guardian, succession or recovery-session mutation needs the seed key. Plan your UX around holding the unlocked key for the session, not prompting per action.

**Whose PIN?** The _signer's_. When a guardian acts on someone else's account (`POST /auth/pin-reset/vote`, `POST /recovery/submit`, `POST /succession/votes`) it is the guardian's own `password` that is required, never the owner's.
...
| `POST /succession/beneficiaries`        | `beneficiary-register`    | `beneficiary_username`                  | Account owner | **yes**                     |
| `DELETE /succession/beneficiaries/{id}` | `beneficiary-delete`      | `{id}` from the path                    | Account owner | **yes**                     |
| `POST /succession/shares`               | `share-assign`            | `beneficiary_id`, `item_id`             | Account owner | **yes**                     |
| `DELETE /succession/shares/{id}`        | `share-delete`            | `{id}` from the path                    | Account owner | **yes**                     |
| `POST /succession/votes`                | `succession-release-vote` | owner's `user_address`, `release_cycle` | Guardian      | **yes**                     |
...
8. **A guardian's release-vote signature needs `owner_user_address` _and_ `owner_release_cycle`, both from `GET /recovery/guardianships`.** They appear only once the invitation is accepted, and no other endpoint returns either — `GET /succession/status` shows the caller's own switch, never the ones they guard. Cache the address; **re-read the cycle before every vote**, because a cancelled countdown increments it and a signature bound to the wrong cycle is refused with `401`.
...
14. **Cross-user reads are `404`, not `403`.** A secret, beneficiary or share belonging to someone else is indistinguishable from one that does not exist.
15. **Only the auth endpoints enforce field-level validation.** `/sign-up`, `/sign-in` and `/auth/verify` run struct validation (missing required field ⇒ `400 INVALID_BODY`); the recovery, succession, secrets and users handlers do not — missing fields surface later as `400 BAD_REQUEST`, `401 INVALID_CREDENTIALS` or `404 NOT_FOUND` from the service layer. Validate client-side rather than relying on a specific code.
16. **`keys_rotated` is misnamed: no client can rotate encryption keys.** They are written once at enrolment and there is no endpoint that changes them, so the flag's namesake condition is unreachable and `dropped_shares` never appears on a registration response. What `keys_rotated: true` actually reports is a beneficiary whose **account was deleted** — `username` and `user_uuid` come back empty, share assignment is blocked with `400`, and re-registering is impossible because the username no longer resolves. `DELETE /succession/beneficiaries/{id}` is the only exit. Surface it as an action item ("this heir closed their account"), not as a re-wrap prompt, and do not build a re-key flow behind it ([§12](./front-end-endpoints.md#12-succession-endpoints)).
...
28. **Ten lists page; `GET /secrets` and `GET /succession/inheritances` do not** ([§3.1](./front-end-endpoints.md#31-pagination)). Send `?limit=` (1–200, default 50) and follow `next_cursor` until `has_more` is `false` — a short page is not the last page, and a cursor is opaque, so never build or parse one yourself. The vault listing is the deliberate exception: it returns everything because the client recomputes the Merkle root over every blob, which is also why `?fields=meta` is unpaginated. Render the vault index from `?fields=meta` and fetch full ciphertext only when you need it; that listing's `ciphertext_sha256` is for change detection, **not** verification — hash the bytes you received. `GET /succession/inheritances` is unpaginated for a different reason: an heir has a handful of inheritances at most, and the listing is filtered after the query by on-chain release state, which paging would slice into ragged pages.

29. **Anchoring is two writes, and the order is not negotiable.** `PUT /succession/anchors/{epoch}` stores the ordered leaf hashes behind the root you are about to anchor, and it must land **before** you submit the userOp. Leaves with no root on-chain are harmless and correctable — re-upload the new set at the same epoch and it replaces the old one — but a root with no leaves is permanent: the epoch freezes on-chain and an heir has no sibling hashes to build a proof from. The only unretryable `409` is contradicting the chain, once that epoch is genuinely anchored ([§12](./front-end-endpoints.md#12-succession-endpoints)).

30. **A document's deltas are never covered by the anchored root.** The Merkle leaf commits to the **snapshot** only, so everything `…/items/{id}/updates` returns for an heir was appended after the compaction that produced the hashed bytes. You need it — a document without its deltas is stale — but "verified as of the owner's last save, plus later edits that carry no proof" is the honest presentation. A single "verified ✓" over the merged document is a false claim, and it is the easiest one to make by accident.

31. **Never render a verification verdict this API gave you, because it never gives one.** No response carries a `verified` field. Hash verification is the heir's client computing the leaf, rebuilding the root from the retained leaf set, and comparing it against `ProofRegistry` read **directly from the chain** — which is why `GET /succession/inheritances` hands you `smart_account_address`. A root fetched from this server proves nothing, since this server is exactly what the verification exists to be independent of.
