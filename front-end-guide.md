...
| `PUT`    | Idempotent full replacement of a singleton at a fixed URL. Sending it twice leaves the same state as sending it once. | `/users/username`, `/notes/{id}`, `/documents/{id}/key`, `/connections/{id}/keys` |
...
19. **Removing a device ends its sessions at once.** A device list with a "remove" action is real: the removed device's tokens stop working on their next request ([§5.5](#55-jwt-usage)).
...
24. **A `404` on a route you expect to work may be a scope.** A limited device gets `404` for the routes of scopes it lacks and for deletes. Check the device's scopes before reporting "not found" to the user.
