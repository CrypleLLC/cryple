...
9. **Every DELETE now requires a body.** It carries the action signature that authorizes the call, so `DELETE /users` and `DELETE /secrets/{id}` — which used to accept an absent body — now answer `400 INVALID_BODY` without one. Standard Mode accounts still omit `password`; they send the signature fields alone. **One exception, added 2026-09-10:** `DELETE /files/{id}/upload` takes no body, because it carries no signed action — it abandons a `pending` upload the same token created, and cannot reach a stored file.
